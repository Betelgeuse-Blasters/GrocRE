...
export default function NewPostModal({ meals, onOk }) {
	...
	function onFinish(values) {
		...
		if (values.photos.length > 0) {
			...
			const photosArr = [];
			fileList.map(() => {
				photosArr.push(URL.createObjectURL(new Blob(fileList)).replace("blob:", ""));
			});
			fileList = photosArr;
	...
	return (
		...
		>
			...
			<Form.Item name="mealSelect" label="Meal/Plan">
				<Select onChange={handleSelect} options={mealNames}></Select>
			</Form.Item>
			<Form.Item
				label="Description"
				name="description"
